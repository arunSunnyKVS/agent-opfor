...
export function registerRunCommand(program: Command): void {
  program
    ...
    .option("--out-dir <path>", `Report output directory (default: ${ASTRA_REPORTS_DIR})`, ASTRA_REPORTS_DIR)
    .action(async (opts: {
      config?: string;
      attacks?: string;
      env?: string;
      ...
      const forcedMode: UnifiedMode | null =
        opts.mcp
          ? "mcp"
          : opts.agent
            ? "agent"
      ...
      let mode: UnifiedMode | null =
        forcedMode ||
        (parsed.mode === "mcp" ? "mcp" : parsed.mode === "agent" ? "agent" : null);
      ...
      if (mode === "mcp") {
        ...
        const paths = await runAgentAttacksFromFile({ input: attacksPath, outputDir: resolvedOutDir });
        console.log(`\nReports:`);
